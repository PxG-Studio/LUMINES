...
export class UnityBridge {
  ...
  static async loadInto(
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        setTimeout(async () => {
          try {
            const { UnityMessagingBus } = await import("@/wis2l/runtime/unityBridge/UnityMessagingBus");
            UnityMessagingBus.setUnityInstance(this.instance);
            
            const { setupUnityEventListeners } = await import("@/wis2l/runtime/unityBridge/RuntimeEvents");
  ...
  static destroy(): void {
    ...
    if (typeof window !== "undefined") {
      Promise.all([
        import("@/wis2l/runtime/unityBridge/UnityMessagingBus").then(({ UnityMessagingBus }) => {
          UnityMessagingBus.reset();
        }).catch(() => {}),
        import("@/wis2l/runtime/unityBridge/RuntimeEvents").then(({ cleanupUnityEventListeners }) => {
